...
Utilizando \textit{-c} se indica el número de CPUs que se desean estresar, que en el ejemplo anterior serán cuatro. Mientras que con \textit{-t} se indica el tiempo que van a estar los cores estresados en segundos, que en este caso son $120$ segundos.
...
Estos ficheros imprimirán por el terminal cuatro valores diferentes que se utilizan para valorar la eficiencia del entrenamiento. Dichos valores son: \textit{Accuracy}, \textit{Trainning accuracy}, \textit{Precision} y \textit{Recall}. 
...
Salvo en el caso de \textit{Trainning accuracy}, habrá que utilizar las predicciones hechas sobre las entradas del \textit{dataset} que se utilizan para probar el modelo. Para el \textit{Trainning accuracy} necesitaremos las predicciones de los datos con los que ha entrenado el propio modelo.

\begin{listing}[htb]
    \caption{Obtención de los valores de \textit{Accuracy}, \textit{Trainning accuracy}, \textit{Precision} y \textit{Recall}.}{}
    \label{lst:get_APR}
...
\centering
    ...
    \cline{2-9}
     & \multicolumn{2}{c}{\textbf{Idle}} & \multicolumn{2}{c}{\textbf{1 CPUs}} & \multicolumn{2}{c}{\textbf{2 CPUs}} & \multicolumn{2}{c}{\textbf{4 CPUs}}\\
...
\centering
    ...
    \hline
    Modelo & CV Accuracy (\%) & Trainning accuracy (\%) & \multicolumn{4}{c}{Accuracy (\%)} \\
    \cline{4-7}
...
Otro ejemplo de esto puede ser el caso en el que se ejecuta el modelo de Random Forest, teniendo el parámetro de \textit{n\_jobs} igual a cuatro para utilizar todos los cores disponibles, a la vez que \textit{stress} para estresar las cuatro CPUs. Si observamos mediante el comando \textit{htop} los procesos, se podrá ver como se distribuyen los cores para que puedan ejecutar a la vez tanto los procesos de \textit{stress} como los del modelo de aprendizaje. Por lo tanto \textit{stress} utilizará en total dos CPUs y Random Forest hará lo mismo, pese a que a ambos se les indicó que utilizasen todas las CPUs de la Raspberry Pi. Este ejemplo se puede ver en la Figura~\ref{figura:stress_rForest}, siendo la CPU total que utiliza Random Forest la que se indica en el primer proceso que muestra \textit{htop} en la imagen. Al sumar el resto de procesos de RF su valor de CPU es igual al del primer proceso.
 ...
 \begin{figure}[htb]
    \centering
    \includegraphics[ width=16cm, keepaspectratio]{img/rForest_stress.png}
...
Tras haber observado los resultados obtenidos en la ejecución de cada uno de los modelos en diferentes situaciones de estrés en ambos dispositivos con los dos \textit{datasets}, se ha podido comprobar que a pesar de la menor capacidad de la Raspberry Pi, esta consigue un comportamiento bastante bueno.
