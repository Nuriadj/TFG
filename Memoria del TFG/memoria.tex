...
El sistema operativo que ofrece es Raspbian Pi OS, que es una versión adaptada de Debian. Sin embargo permite utilizar otros sistemas.
...
\section{Entorno de desarrollo: Miniforge}
...
Ofrece casi lo mismo que Anaconda pero es mucho más ligero, lo que lo hace idóneo para poder desarrollar el proyecto en el dispositivo utilizado. Además de que permite tener un mayor control sobre los paquetes que se instalan y ser más sencillo replicar un entorno concreto.
...
El primer paso para esto fue montar adecuadamente la Raspberry Pi conforme las instrucciones lo indican añadiendo el ventilador, carcasa... Una vez está listo el hardware hay que instalar el software necesario para la aplicación de modelos de machine learning.

Una de las prioridades fue instalar el gestor de paquetes Miniconda de forma que permitiese crear un entorno virtual con una versión de Python superior a la 3.7. Sin embargo, debido a la arquitectura de 32-bit empleada por el sistema operativo instalado por defecto en la Raspberry (Raspbian Pi OS), no era posible instalar una versión de Python superior a la 3.6, pues para esas versiones se requería una arquitectura de 64-bit.

Por lo que fue necesario instalar Ubuntu 21.10 cuya arquitectura es de 64-bit. Aún así, tampoco se pudo instalar Miniconda con una versión de Python 3.8 o 3.9. La solución recayó en instalar Miniforge que instala un administrador de paquetes conda, muy similar a la función que desempeña Miniconda.
