...
Comenzando por cambiar el sistema operativo, en vez de utilizar el que viene por defecto, Raspbian Pi OS, se instaló Ubuntu 21.10.
...
Con estos modelos generados se procedió a comprobar la eficiencia de la Raspberry a la hora de generarlos. Para ello se definieron tres niveles de saturación: el nivel bajo consistía en estresar una única cpu, el nivel medio, dos cpus y por último en el nivel alto se estrasaban todas las cpus de la Raspberry, es decir, cuatro cpus. 
...
A los tres modelos anteriormente generados se sumó un cuarto basado en Random forest tree, realizando tanto una versión que utilizaba validación cruzada como sin ella. El motivo de añadir este modelo fue para ayudar a comprobar si el comportamiento de la Raspberry bajo diferentes niveles de saturación era más o menos lo esperado para cada uno de los modelos, puesto que sabemos que a mayor cantidad de cpus estresadas Random tree forest debería de aumentar su tiempo de ejecución.
